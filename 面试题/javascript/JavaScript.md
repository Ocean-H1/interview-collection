...
- [175. Token 优缺点？](#175-token-优缺点)
- [176. 如何判断dom元素是否在可视区域?](#176-如何判断dom元素是否在可视区域)
- [177. Js中如何进行大对象深度对比？](#177-js中如何进行大对象深度对比)
- [178. JS中执行100万个任务，如何保证浏览器不卡顿?](#178-js中执行100万个任务如何保证浏览器不卡顿)


### 1. 介绍 js 的基本数据类型。

```
js 包括5种基本数据类型和1种复杂数据类型：Undefined、Null、Boolean、Number、String、Object。还有在 ES6 中新增的 Symbol 和 ES10 中新增的 BigInt 类型。
...
web 资源的缓存策略一般由服务器来指定，可以分为两种，分别是强缓存策略和协商缓存策略。
	...
	1. 使用强缓存策略时，如果缓存资源有效，则直接使用缓存资源，不必再向服务器发起请求。强缓存策略可以通过两种方式来设置，分别是 http 头信息中的 Expires 属性和 Cache-Control 属性。
...
一般来说只需要设置其中一种方式就可以实现强缓存策略，当两种方式一起使用时，Cache-Control 的优先级要高于 Expires 。
	...
	2. 使用协商缓存策略时，会先向服务器发送一个请求，如果资源没有发生修改，则返回一个 304 状态，让浏览器使用本地的缓存副本。
...
![cache-control](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/80bea8b2247b4c73b8bb9e874b7252a7~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image?)

![协商缓存字段](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3dd35611e8e74e0590acef4ba1656e9f~tplv-k3u1fbpfcp-zoom-in-crop-mark:4536:0:0:0.image?)

两者主要区别：

- 强缓存不需要发送请求到服务器，而协商缓存需要；所以强缓存资源的更新浏览器不能知道，而协商缓存资源的更新浏览器可以知道
- 大部分 web 默认用协商缓存
...
（1）第一个方面是**在模块定义时对依赖的处理不同**。AMD 推崇依赖前置，在定义模块的时候就要声明其依赖的模块。而 CMD 推崇
就近依赖，只有在用到某个模块的时候再去 require。

（2）第二个方面是**对依赖模块的执行时机处理不同**。首先 AMD 和 CMD 对于模块的加载方式都是异步加载，不过它们的区别在于
...
```
概念：可以理解为令牌，作为访问资源的凭证

认证流程：
1. 用户输入用户名和密码，客户端发送给服务器
2. 服务器验证用户名和密码，如果正确，就返回一个签名过的token（本质上就是一个长字符串），浏览器客户端拿到这个token
3. 后续每次请求中，浏览器会把token作为HttpRequest Header发送给服务器，服务器验证签名是否有效，如果有效代表认证成功，可以返回客户端需要的数据

特点：客户端中的token中自己保留有大量信息，服务器没有存储这些信息
...
```
概念: JWT（Json Web Token），跟纸上就是一个字符串书写规范，用来在用户和服务器之间传递安全可靠的信息

认证流程: 在目前的前后端分离开发过程中，使用token鉴权机制用于身份验证是最常见的方案，大致流程如下:
	1. 服务器验证用户名和密码正确的时候，给用户颁发一个令牌，这个令牌作为后续用户访问一些接口的凭证
	2. 后续访问会根据这个令牌判断用户是否有权限进行访问

组成: `token`，分成了三部分，头部(Header),载荷(Payload),签名(Signature),并以`.`进行拼接。注意，其中header和payload都是以JSON格式存放数据，只是进行了编码，所以尽量使用HTTPS传输，且不要把过于敏感的信息放入
	1. Header（头部）: 通常包含了两部分，`type`：代表token的类型，,`alg`: 代表的使用的hash算法，例如`HMAC SHA256或RSA`。{ "alg": "HS256", "typ": "JWT" } 这会被经过base64Url编码形成第一部分
	2. Payload(载荷即消息体)：这里会存放实际的内容，也就是`Token`的数据声明，例如用户的`id`和`name`,默认情况下也会携带令牌的签发时间`iat`,也可以通过`exp`设置过期时间,例如{ "sub": "1234567890", "name": "John Doe", "admin": true } 同样经过Base64Url编码后形成第二部分
	3. Signature 使用 header中指定的算法将编码后的header、编码后的payload、和secret密钥进行加密。例如使用HMAC SHA256算法，大致流程类似于: HMACSHA256( base64UrlEncode(header) + "." + base64UrlEncode(payload), secret) ，这个signature字段用来确认JWT信息的发送者是谁，并保证信息没有被修改。
```
...
1. 解析JWT
2. 根据生成签名时使用的密钥secret和加密算法，判断签名Signature是否正确
3. 如果签名正确，说明payload中的信息是可信的，然后根据exp字段判断JWT token是否过期，再根据isVip字段判断是否是Vip

总结： JWT版没有查询数据库，所需要的基础信息可以直接放到JWT里，服务端只需要判断签名是否正确就可以判断用户是否有权限访问该接口，但JWT里的内容也不是无限多的，其他更多的信息我们就可以通过id去查数据库。
...
```
1. 支持跨域访问: Cookie是不允许跨域的，但token支持
2. 无状态: token无状态，session有状态
3. 去耦合: token可以在任何地方生成，不需要绑定到一个特定的身份验证方案，只要在你的API被调用的时候，进行token生成调用即可
4. 更适用于移动应用，Cookie不支持手机端访问
5. 性能更好: 在网络传输的过程中，性能更好
```

缺点：

```
1. 占带宽: 正常情况下要比session_id更大，需要消耗更多流量，挤占更多带宽
2. 无法在服务端注销，所以很难解决劫持问题
3. 性能问题: 需要花费更多的CPU开销来验证签名，对有着严格性能要求的web应用，并不理想，尤其
```

### 176. 如何判断dom元素是否在可视区域?

### 177. Js中如何进行大对象深度对比？

### 178. JS中执行100万个任务，如何保证浏览器不卡顿?